...
events with stable persisted names, or the freedom to move between state and event stream. That is
what this family adds — and this package is the layer where none of that has met a framework yet.
...
- Child entities derive from `Entity<TKey, TState>`, keep an **internal** constructor and raise
  ...
  have nothing to raise into.

`GaWeCodes.Thessera.Testing` turns all of these from startup errors into a single test.
...
prefixes every event stream key, an event name is written into every persisted event row, and both
