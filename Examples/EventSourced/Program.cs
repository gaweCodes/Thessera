...
var connectionString = Environment.GetEnvironmentVariable("THESSERA_EXAMPLE_POSTGRES")
    ?? EventSourcedApplication.DefaultConnectionString;

await using var app = await EventSourcedApplication.StartAsync(connectionString);
...
static void Print<TResult>(Result<TResult> result)
    ...
    Console.WriteLine(JsonSerializer.Serialize(ResultEnvelope.From(result), EventSourcedJson.Options));

static void PrintEnvelope(ResultEnvelope envelope) =>
    Console.WriteLine(JsonSerializer.Serialize(envelope, EventSourcedJson.Options));
