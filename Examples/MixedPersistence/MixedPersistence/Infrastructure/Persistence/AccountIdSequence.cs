...
public sealed class AccountIdSequence : IAccountIdSequence
{
    ...
    public AccountId ReserveNext() => new(Interlocked.Increment(ref _current));

    public void TryRelease(AccountId id)
    {
        if (id.IsEmpty)
        {
            return;
