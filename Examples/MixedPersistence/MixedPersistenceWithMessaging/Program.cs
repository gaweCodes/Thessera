...
var connectionString = Environment.GetEnvironmentVariable("THESSERA_EXAMPLE_POSTGRES")
    ?? MixedPersistenceWithMessagingApplication.DefaultConnectionString;
var rabbitMqUri = Environment.GetEnvironmentVariable("THESSERA_EXAMPLE_RABBITMQ")
    ?? MixedPersistenceWithMessagingApplication.DefaultRabbitMqUri;
...
static void Print<TResult>(Result<TResult> result)
    ...
    Console.WriteLine(JsonSerializer.Serialize(ResultEnvelope.From(result), MixedPersistenceWithMessagingJson.Options));

static void PrintEnvelope(ResultEnvelope envelope) =>
    Console.WriteLine(JsonSerializer.Serialize(envelope, MixedPersistenceWithMessagingJson.Options));
